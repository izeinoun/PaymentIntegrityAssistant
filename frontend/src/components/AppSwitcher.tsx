...
// URLs come from the committed config/appUrls (switch by build mode).
import { ClipboardCheck, KeyRound, ShieldCheck, Siren } from 'lucide-react'
import { APP_URLS } from '../config/appUrls'

// The assistant app is "current", so it's excluded from its own switcher.
const APPS = [
...
export default function AppSwitcher() {
  return (
    <nav className="flex items-center gap-1" aria-label="Applications">
      {APPS.map(({ key, label, href, icon: Icon }) => (
        <a
          key={key}
          href={href}
          title={`Open ${label}`}
