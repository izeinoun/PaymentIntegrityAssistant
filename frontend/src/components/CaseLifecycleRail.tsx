// Workflow lifecycle stepper, driven by the backend Case Guidance Engine
// (server/app/services/case_guidance_service.py). Horizontal on the cockpit
// header; vertical option available. Mirrors the PayGuard CaseLifecycleRail.
import { Check, AlertTriangle, Minus } from 'lucide-react'
import type { LifecycleStep, LifecycleStepState } from '../api/types'
...
const CONNECTOR: Record<LifecycleStepState, string> = {
  completed: 'bg-green-400', current: 'bg-gray-200', blocked: 'bg-gray-200',
  upcoming: 'bg-gray-200', skipped: 'bg-gray-100',
}

function Dot({ step, index }: { step: LifecycleStep; index: number }) {
  return (
    <div
      className={`flex items-center justify-center w-6 h-6 rounded-full border text-[11px] font-bold flex-shrink-0 ${DOT[step.state]}`}
        ...
        {steps.map((step, i) => (
          <li key={step.key} className="flex gap-3">
            ...
            <div className="pb-3 -mt-0.5">
              ...
              {step.detail && <p className="text-xs text-amber-700 mt-0.5">{step.detail}</p>}
            </div>
          </li>
        ))}
      </ol>
      ...
      {steps.map((step, i) => (
        <div key={step.key} className="flex items-start flex-1 last:flex-none">
          <div className="flex flex-col items-center text-center" style={{ minWidth: 0 }}>
            ...
            {step.detail && <p className="text-[10px] text-amber-700 mt-0.5 px-1 leading-tight">{step.detail}</p>}
          </div>
          {i < steps.length - 1 && <div className={`h-px flex-1 mt-3 mx-1 ${CONNECTOR[step.state]}`} />}
        </div>
      ))}
    </div>
  )
